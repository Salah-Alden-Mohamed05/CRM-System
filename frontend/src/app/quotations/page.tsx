...
import {
  ...
  ChevronDown, ChevronUp, Plane, Truck, Star, Hash, Mail, Copy, Send
} from 'lucide-react';
...
function QuotationsContent() {
  ...
  const [emailForm, setEmailForm] = useState({ recipientEmail: '', recipientName: '', subject: '', body: '' });
  ...
  const openEmailDialog = (q: any) => {
    setEmailForm({
      recipientEmail: q.client_contact_email || '',
      recipientName: q.client_contact_name || '',
      subject: `Quotation ${q.quotation_number} – ${q.customer_name}`,
      body: `Dear ${q.client_contact_name || 'Sir/Madam'},\n\nPlease find attached our quotation ${q.quotation_number} for your review.\n\nTotal Amount: ${q.currency} ${Number(q.total_amount).toLocaleString()}\nValid Until: ${q.valid_until ? new Date(q.valid_until).toLocaleDateString() : 'Upon request'}\n\nPlease do not hesitate to contact us if you have any questions.\n\nBest regards`,
    });
    setEmailDialog(q);
  };

  const handleSendEmail = async () => {
    if (!emailDialog || !emailForm.recipientEmail) return;
    setEmailSending(true);
    try {
      await quotationsAPI.sendEmail(emailDialog.id, emailForm);
      showToast(isRTL ? 'تم إرسال البريد الإلكتروني بنجاح!' : 'Email sent successfully!');
      setEmailDialog(null);
      fetchData();
    } catch (e: any) {
  ...
  const handleDuplicate = async (id: string) => {
    setDuplicating(id);
    try {
      await quotationsAPI.duplicate(id);
      showToast(isRTL ? 'تم نسخ عرض السعر!' : 'Quotation duplicated!');
      fetchData();
                    ...
                    <div className="flex flex-col gap-2 items-end flex-shrink-0">
                      ...
                      <div className="flex gap-1 flex-wrap justify-end">
                        <button onClick={() => openPDFPreview(q)}
                          className="text-xs text-gray-500 hover:bg-gray-100 px-2 py-1.5 rounded-lg flex items-center gap-1 transition-colors"
                          title="Preview PDF">
                          <Printer className="w-3.5 h-3.5" />
                        </button>
                        {canCreate && (
                          <button onClick={() => openEmailDialog(q)}
                            className="text-xs text-blue-600 hover:bg-blue-50 px-2 py-1.5 rounded-lg flex items-center gap-1 transition-colors border border-blue-200"
                            title="Send Email">
                        ...
                        {canCreate && (
                          <button onClick={() => handleDuplicate(q.id)}
                            ...
                            className="text-xs text-indigo-600 hover:bg-indigo-50 px-2 py-1.5 rounded-lg flex items-center gap-1 transition-colors border border-indigo-200 disabled:opacity-50"
                            title="Duplicate">
                            {duplicating === q.id ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Copy className="w-3.5 h-3.5" />}
                            <span className="hidden sm:inline">{isRTL ? 'نسخ' : 'Copy'}</span>
                          </button>
                        )}
                        {canCreate && (
                          <button onClick={() => { setEditData(q); setShowForm(true); }}
                            className="text-xs text-violet-600 hover:bg-violet-50 px-2 py-1.5 rounded-lg flex items-center gap-1 transition-colors">
                            <Edit3 className="w-3.5 h-3.5" />
                          </button>
                        )}
                        {q.deal_id && (
                          <button onClick={() => router.push(`/deals/${q.deal_id}`)}
                            className="text-xs text-blue-600 hover:bg-blue-50 px-2 py-1.5 rounded-lg flex items-center gap-1 transition-colors">
                            <ExternalLink className="w-3 h-3" />
                          </button>
                        )}
                        {canDelete && (
                          <button onClick={() => setDeletingId(q.id)}
                            className="text-xs text-red-400 hover:bg-red-50 hover:text-red-600 px-2 py-1.5 rounded-lg transition-colors">
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                        )}
                      </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
              <div className="flex items-center justify-between p-5 border-b">
                <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                  ...
                  {isRTL ? 'إرسال عرض السعر بالبريد الإلكتروني' : 'Send Quotation by Email'}
                </h2>
                <button onClick={() => setEmailDialog(null)} className="p-1.5 hover:bg-gray-100 rounded-lg">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="p-5 space-y-4">
                <div className="bg-blue-50 rounded-xl p-3 text-sm text-blue-800">
                  <strong>{emailDialog.quotation_number}</strong> · {emailDialog.customer_name} · {emailDialog.currency} {Number(emailDialog.total_amount).toLocaleString()}
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">{isRTL ? 'البريد الإلكتروني للمستلم *' : 'Recipient Email *'}</label>
                  <input type="email" value={emailForm.recipientEmail} onChange={e => setEmailForm(f => ({ ...f, recipientEmail: e.target.value }))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400" placeholder="customer@example.com" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">{isRTL ? 'اسم المستلم' : 'Recipient Name'}</label>
                  <input type="text" value={emailForm.recipientName} onChange={e => setEmailForm(f => ({ ...f, recipientName: e.target.value }))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">{isRTL ? 'موضوع الرسالة' : 'Subject'}</label>
                  <input type="text" value={emailForm.subject} onChange={e => setEmailForm(f => ({ ...f, subject: e.target.value }))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">{isRTL ? 'نص الرسالة' : 'Message Body'}</label>
                  <textarea rows={5} value={emailForm.body} onChange={e => setEmailForm(f => ({ ...f, body: e.target.value }))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 resize-none" />
                </div>
                {emailDialog?.email_sent_count > 0 && (
                  <p className="text-xs text-gray-500">Previously sent {emailDialog.email_sent_count} time(s). Last to: {emailDialog.last_email_recipient}</p>
                )}
              </div>
              <div className="flex gap-3 p-5 border-t">
                <button onClick={() => setEmailDialog(null)} className="flex-1 py-2.5 border border-gray-300 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50">
                  {isRTL ? 'إلغاء' : 'Cancel'}
                </button>
                <button onClick={handleSendEmail} disabled={emailSending || !emailForm.recipientEmail}
